...
typedef __gnu_cxx::hash_map<kmer_int_type_t, Path_n_count_pair, hashme, eqstr> precomputations;
...
public:
    ...
    void build_graph(const string& reads_fasta_filename, bool reassembleIworm = false, bool useKmers = false, float min_ratio_non_error = 0.0);
    ...
    bool fast_pruning(unsigned int min_count, float min_entropy, bool prune_error_kmers, float min_ratio_non_error);
    ...
    void parallel_compute_sequence_assemblies(float min_connectivity = 0.2, 
...
private:
    ...
    void populate_kmers_using_mmap(string filename, float min_ratio_non_error = 0.0);
    ...
    void parallel_compute_sequence_assemblies(KmerCounter& kcounter, float min_connectivity, 
                                     unsigned int min_assembly_length, unsigned int min_assembly_coverage,
                                     bool write_coverage, string coverage_output_filename);

    void parallel_compute_sequence_assemblies_same_results(KmerCounter& kcounter, float min_connectivity, 
                                     unsigned int min_assembly_length, unsigned int min_assembly_coverage,
                                     bool write_coverage, string coverage_output_filename);
