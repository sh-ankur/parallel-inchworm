  ...
  st = time(NULL);
    ...
    if (reads_fasta_filename.length()) {
        // let inchworm do it (pretty slow)
    		irke.build_graph(reads_fasta_filename, reassembleIworm, false, min_ratio_non_error);

    } 
    else if (kmers_fasta_filename.length()) {
    	// read an existeing kmer catalog (such as from Meryl)
        irke.build_graph(kmers_fasta_filename, reassembleIworm, true, min_ratio_non_error);  
        ...
        for (unsigned int i = 0; i < kmer_files_list.size(); i++) {
            ...
            irke.build_graph(kmer_filename, reassembleIworm, true, min_ratio_non_error);
        }
    }
    ...
    if (search_CDS_flag) { // define the 'oracle' transcript set
        ...
        if (min_kmer_count > 1 || min_any_entropy > 0 || prune_error_kmers) {
            ...
            st_p = time(NULL);
            irke.fast_pruning(min_kmer_count, min_any_entropy, prune_error_kmers, min_ratio_non_error);
            ed_p = time(NULL);
            cerr << "Pruning took " << ed_p - st_p << " seconds" << endl;
        }
        ...
        if (run_inchworm) { // note this would have to be true since either or both are required.
            ...
            cerr << "Done sorting kmers in " << end - start << " seconds" << endl;
            irke.parallel_compute_sequence_assemblies(MIN_CONNECTIVITY_RATIO, MIN_ASSEMBLY_LENGTH, MIN_ASSEMBLY_COVERAGE, WRITE_COVERAGE, COVERAGE_OUTPUT_FILENAME);
        }
    ...
    cerr << "Total time for inchworm: " << ed - st << " seconds" << endl;
