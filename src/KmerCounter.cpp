...
bool KmerCounter::fast_pruning(unsigned int min_count, float min_entropy, bool prune_error_kmers, float min_ratio_non_error)
{
    int tid;
    time_t start, end;
    start = time(NULL);
    unsigned int count_pruned = 0;
    unsigned int num_kmers = _kmer_counter.size();

    //vector<Kmer_counter_map_iterator> kmer_list;
    vector<Kmer_Occurence_Pair> kmer_list; 
    vector<kmer_int_type_t> prune_list;
    Kmer_counter_map_iterator it;    
    int count = 0;
    for (it = _kmer_counter.begin(); it != _kmer_counter.end(); it++) {
    ...
    #pragma omp parallel num_threads(20) shared(kmer_list, min_count, min_entropy, min_ratio_non_error, count_pruned) private(tid, prune_list)
    {    
        tid = omp_get_thread_num();
        kmer_int_type_t kmer;
        unsigned int count;
        for(unsigned int j = tid ; j < kmer_list.size() ; j += 20)
        {
            kmer = kmer_list[j].first;
            count = kmer_list[j].second;
            ...
            if (prune_error_kmers)
            {
              ...
              for (unsigned int i = 0; i < candidates.size(); i++)
              {
                if (candidates[i].second)
                {
                  ...
                  {
                    prune_list.push_back(candidates[i].first);
                  }
                }
              }
            } 
        ...
        #pragma omp critical(HashMap)
        {
          ...
          for(unsigned int k = 0 ; k < prune_list.size() ; k++)
          {
            it = find_kmer(prune_list[k]);
            if(it->second > 0)
            { 
              count_pruned++;
              it->second = 0;
    ...
    if (count_pruned > 0) 
    {  
      cerr << "Pruned " << count_pruned << " kmers from catalog." << "in "  << end - start << " seconds" << endl;
      return(true);
    }
    else {
        return(false);
...
bool KmerCounter::set_forward(kmer_int_type_t kmer_val) {
    ...
    Kmer_counter_map_iterator it = find_kmer(kmer_val);
      if (it != _kmer_counter.end())
      {
        it->second = it->second | 1073741824;
        ret = true;
      }
    }
...
bool KmerCounter::set_reverse(kmer_int_type_t kmer_val) {
    ...
    Kmer_counter_map_iterator it = find_kmer(kmer_val);
      if (it != _kmer_counter.end())
      {
        it->second = it->second | 2147483648;
        ret = true;
      }
    }
...
bool KmerCounter::set_seed(kmer_int_type_t kmer_val) {
    ...
    Kmer_counter_map_iterator it = find_kmer(kmer_val);
      if (it != _kmer_counter.end())
      {
        it->second = it->second | 3221225472;
        ret = true;
      }
    }
...
vector<Kmer_Occurence_Pair> KmerCounter::get_forward_kmer_candidates_unsorted(kmer_int_type_t seed_kmer, bool getZeros) {
  ...
  for (kmer_int_type_t i=0; i<4; i++)
  {
    ...
    candidate.first = forward_prefix | i;
        ...
        Kmer_counter_map_iterator it = _kmer_counter.find(candidate.first);

        if(it != _kmer_counter.end() && is_set_right(it->second))
            candidate.second = 0;
    else
...
vector<Kmer_Occurence_Pair> KmerCounter::get_reverse_kmer_candidates_unsorted(kmer_int_type_t seed_kmer, bool getZeros) {
  ...
  for (kmer_int_type_t i=0; i<4; i++)
  {
    ...
    candidate.first = (i << (_kmer_length*2-2)) | reverse_suffix;
        ...
        if(it != _kmer_counter.end() && is_set_left(it->second))
            candidate.second = 0;
        else
            candidate.second = get_kmer_count(candidate.first);
    if (candidate.second || getZeros)
