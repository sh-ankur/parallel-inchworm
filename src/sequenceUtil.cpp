...
#define KMER_SIZE 25
#define CHECK_LEFT 2147483648
#define CHECK_RIGHT 1073741824

bool is_set_left(unsigned int count)
{
	return ((count & CHECK_LEFT) > 0) ? true : false;
}

bool is_set_right(unsigned int count)
{
	return ((count & CHECK_RIGHT) > 0) ? true : false;
}

void set_left(unsigned int &count)
{
	count = count | CHECK_LEFT;
}

void set_right(unsigned int &count)
{
	count = count | CHECK_RIGHT;
}
...
kmer_pair extract_kmer(string str) {
	//cout << "Extract:"<< str << "---size:" << str.size() << endl;
	int count = 0, j;
	kmer_int_type_t kmer_entry;
	kmer_pair pair;
	char temp[KMER_SIZE + 1];
	for (int i = 0, j = 0; i <= str.size(); i++) {
		if (count == 0 && (int) str[i] == 10 && i != str.size() - 1) {
			temp[j] = '\0';
			count = atoi(temp);
			//cout << "Count_Test:" << count << endl;
			j = 0;
		} else if (i == str.size()) {
			//cout << "test:" << endl;
			temp[j] = '\0';
			if (j == KMER_SIZE) {
				//cout << "temp_seq:" << temp << endl;
				kmer_entry = kmer_to_intval(temp);
				//cout << "SeqUtil:" << kmer_entry << endl;
			} else
				kmer_entry = -1;
			pair = make_pair(kmer_entry, count);
		} else {
			//cout << str[i];
			temp[j++] = str[i];
		}
